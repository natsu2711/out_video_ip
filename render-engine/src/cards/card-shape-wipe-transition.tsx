...
const CONFIG = { ...(__INJ__.CONFIG ?? {}),
  ...
  wipeDur: 0.45,    // 单层扫屏耗时 s：>{(__INJ__.TEXT?.[0] ?? "0.7 读作拉幕布，")}<0.3 只剩闪烁
  ...
  return (
    <AbsoluteFill style={{
      ...
      <div className="scene scene-a" style={{
        ...
        <Host src={hostSrc} />
        <div className="chapter-tag">{(__INJ__.TEXT?.[1] ?? "01 · 讲概念")}</div>
      </div>
      ...
      <div className="scene scene-b" style={{
        ...
        <div className="chapter-tag">{(__INJ__.TEXT?.[2] ?? "02 · 看数据")}</div>
        <div className="chart-card">
          <div className="chart-title">{(__INJ__.TEXT?.[3] ?? "季度营收同比增速")}</div>
          <div className="bars">
          ...
          <div className="chart-src">{(__INJ__.TEXT?.[4] ?? "数据来源：公司 2024 年报")}</div>
        </div>
      </div>
