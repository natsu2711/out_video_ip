// [outvideo] card-sc-runway-ground-skim —— 移植自 video-shotcraft/demos/ui-entrance/runway-ground-skim
    ...
    <div style={{ display: 'flex', height: '100%' }}>
      ...
      <div style={{ width: 860, borderRight: `4px solid ${FAINT}`, padding: '70px 60px 0', background: '#f1f1f0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 26 }}>
          ...
          <div style={{ fontFamily: FONT, fontSize: 56, fontWeight: 800, color: INK }}>{((__INJ__.TEXT3 as string[])?.[2] ?? "ClickUp")}</div>
        </div>
        <div style={{ height: 46 }} />
        ...
        <div style={{ height: 60 }} />
        <div style={{ fontFamily: FONT, fontSize: 38, letterSpacing: 5, color: MID, fontWeight: 600, paddingLeft: 32 }}>{((__INJ__.TEXT2 as string[])?.[1] ?? "SPACES")}</div>
        <div style={{ height: 16 }} />
      ...
      <div style={{ flex: 1, padding: '70px 100px 0', position: 'relative' }}>
        ...
        <div style={{ display: 'flex', gap: 110, fontFamily: FONT, fontSize: 42, color: MID, marginBottom: 60 }}>
          <div>{((__INJ__.TEXT as string[])?.[0] ?? "Product analytics")}</div><div style={{ fontWeight: 700, color: INK }}>ClickUp 3.0</div>
...
export default RunwayGroundSkim;
