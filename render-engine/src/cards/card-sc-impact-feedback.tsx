// [outvideo] card-sc-impact-feedback —— 移植自 video-shotcraft/demos/effects/impact-feedback
// 原动效保留；内容接线：raw 档（可在编排手动用，后续逐张接 __INJ__）
import React from 'react';
const __INJ__: any = (globalThis as any).__OUTVIDEO_CARD__ ?? {};
import { AbsoluteFill, Easing, interpolate, useCurrentFrame } from 'remotion';
import { FakeDashboard, Card, TitleBlock, G } from './_sc/Fixtures';
  ...
  <>
    ...
    <div style={{ position: 'absolute', left: CARD.x, top: CARD.y }}>
      ...
      <div style={{ position: 'absolute', left: 24, bottom: 96 }}>
        <TitleBlock text={((__INJ__.TEXT as string[])?.[0] ?? "IMPACT")} size={92} />
      </div>
    </div>
  </>
);
