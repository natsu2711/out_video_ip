// [outvideo] card-sc-brand-frame-snap —— 移植自 video-shotcraft/demos/effects/brand-frame-snap
...
import { G, FakeDashboard } from './_sc/Fixtures';

const __INJ__: any = (globalThis as any).__OUTVIDEO_CARD__ ?? {};
// 文字槽：TEXT = [DESIGN 模式标签, DEV 模式标签]
const MODE_LABELS: string[] = ((__INJ__.TEXT as string[])?.length ? (__INJ__.TEXT as string[]) : ['DESIGN', 'DEV MODE']);
...
export const BrandFrameSnap: React.FC = () => {
  ...
  // 模式标签
  const label = mode === 'design' ? (MODE_LABELS[0] ?? 'DESIGN') : (MODE_LABELS[1] ?? 'DEV MODE');
