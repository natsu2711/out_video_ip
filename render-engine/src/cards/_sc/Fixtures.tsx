...
// [outvideo] 槽位机制：空白矩形 = 内容槽。slot n 读 __INJ__.SLOTS[n] = {text?, image?}；
// 有文字 → 矩形内渲染大字；有图片 → 铺满矩形；都没有 → 原灰阶占位。
// 矩形本身参与卡的动效（它就是动效里的那个元素），文字/图片跟着矩形一起动。
export const Card: React.FC<{
  w: number; h: number; seed?: number; style?: React.CSSProperties; slot?: number;
}> = ({ w, h, seed = 0, style, slot }) => {
  const __SLOTS__: any = (globalThis as any).__OUTVIDEO_CARD__?.SLOTS ?? null;
  const slotData = slot != null && __SLOTS__ ? __SLOTS__[slot] : null;
  ...
  return (
    <div style={{
      ...
      {slotData?.text ? (
        <div style={{ margin: 'auto', textAlign: 'center', padding: 8,
                      fontWeight: 900, fontSize: Math.max(20, Math.min(w, h) * 0.14),
                      color: G.ink, lineHeight: 1.3,
