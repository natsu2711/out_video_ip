...
// [outvideo] card-sc-pill-chip-slot-cycle-handled —— 移植自 video-shotcraft/demos/typography/pill-chip-slot-cycle-handled
  ...
  return (
    <DesignStage bg="#fbfbfd">
      ...
      >
        ...
        <div style={SIDE}>{((__INJ__.TEXT2 as string[])?.[1] ?? "Your")}</div>
        ...
        <div style={SIDE}>{((__INJ__.TEXT as string[])?.[0] ?? "Handled")}</div>
      </div>
    </DesignStage>
  );
};
...
export default PillChipSlotCycleHandled;
