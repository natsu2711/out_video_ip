// [outvideo] card-sc-cloner-depth-echo —— 移植自 video-shotcraft/demos/ui-entrance/cloner-depth-echo
...
import { G, Card, TitleBlock } from './_sc/Fixtures';
        ...
        >
          ...
          {Array.from({ length: N }, (_, k) => N - k).map((idx) => {
            ...
            return (
              ...
              >
                <Card slot={0} w={520} h={340} seed={3} />
              </div>
            );
          })}
          {/* 本体 */}
          <div style={{ position: 'absolute', transform: `translateZ(0px) scale(${heroScale.toFixed(4)})` }}>
            <Card slot={1} w={520} h={340} seed={3} style={{ boxShadow: '0 10px 36px rgba(31,28,23,0.22)' }} />
          </div>
        </div>
      </div>
    </div>
...
export default ClonerDepthEcho;
