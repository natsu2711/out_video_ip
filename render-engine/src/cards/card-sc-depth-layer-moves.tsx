...
// [outvideo] card-sc-depth-layer-moves —— 移植自 video-shotcraft/demos/camera/depth-layer-moves
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: '#efece6', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, transform: `translateX(${-drive * 0.35}px) scale(1.05)`, filter: 'blur(2px) saturate(0.92)', opacity: 0.85 }}>
        <Img src={staticFile(String(((__INJ__.CONFIG as any)?.image_1) ?? 'textures/live/projects-full.png'))} style={{ position: 'absolute', left: 0, top: -300, width: 2100 }} />
      </div>
      ...
      <div style={{ position: 'absolute', inset: 0, transform: `translateX(${-drive * 1.4}px)`, filter: 'blur(3px)' }}>
        <Img src={staticFile(String(((__INJ__.CONFIG as any)?.image_2) ?? 'textures/live/float-search.png'))} style={{ position: 'absolute', left: 900, top: 150, width: 700, borderRadius: 22, boxShadow: '0 12px 36px rgba(31,28,23,0.18)' }} />
        <Img src={staticFile(String(((__INJ__.CONFIG as any)?.image_3) ?? 'textures/live/card4-hires.png'))} style={{ position: 'absolute', left: 2100, top: 700, width: 380, borderRadius: 12, boxShadow: '0 12px 36px rgba(31,28,23,0.20)' }} />
      </div>
    </AbsoluteFill>
  );
};
