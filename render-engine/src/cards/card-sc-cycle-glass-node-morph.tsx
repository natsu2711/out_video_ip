// [outvideo] card-sc-cycle-glass-node-morph —— 移植自 video-shotcraft/demos/data/cycle-glass-node-morph
...
import { DesignStage, E, lerp } from './_sc/Motion';

const __INJ__: any = (globalThis as any).__OUTVIDEO_CARD__ ?? {};
// TEXT = CONTEXT 横幅；TEXT2 = [SYSTEM LOOP 标题, SUBJECT 主体文字]；TEXT3 = 循环节点标签 ×3
const TXT_CONTEXT = ((__INJ__.TEXT as string[])?.[0] ?? 'CONTEXT');
const TXT_SYSTEM = ((__INJ__.TEXT2 as string[])?.[0] ?? 'SYSTEM LOOP');
const TXT_SUBJECT = ((__INJ__.TEXT2 as string[])?.[1] ?? 'SUBJECT');
const NODE_LABELS = ((__INJ__.TEXT3 as string[])?.length ? (__INJ__.TEXT3 as string[]) : ['SENSE', 'MODEL', 'ACT']);
...
export const CycleGlassNodeMorph: React.FC = () => {
  ...
  const labelData = [
    { label: NODE_LABELS[0] ?? 'SENSE', left: 147, top: 92, start: 124 },
    { label: NODE_LABELS[1] ?? 'MODEL', left: 240, top: 63, start: 129 },
    { label: NODE_LABELS[2] ?? 'ACT', left: 333, top: 92, start: 134 },
  ];
    ...
    <DesignStage bg="#f5f5f2" raster="zoom">
      ...
      >
        ...
        >
          ...
          <GlassNode frame={frame} start={184} end={192} left={92} top={98} label={NODE_LABELS[0] ?? 'SENSE'} />
          <GlassNode frame={frame} start={192} end={200} left={185} top={69} label={NODE_LABELS[1] ?? 'MODEL'} />
          <GlassNode frame={frame} start={200} end={208} left={278} top={98} label={NODE_LABELS[2] ?? 'ACT'} />
...
export default CycleGlassNodeMorph;
