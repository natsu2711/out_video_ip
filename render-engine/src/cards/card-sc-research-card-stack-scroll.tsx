// [outvideo] card-sc-research-card-stack-scroll —— 移植自 video-shotcraft/demos/ui-entrance/research-card-stack-scroll
...
const TITLES: string[] = ((__INJ__.TEXT as string[])?.length ? (__INJ__.TEXT as string[]) : [
...
const AUTHOR_LINE = ((__INJ__.TEXT2 as string[])?.[0] ?? 'A. Author, B. Writer, C. Reader');
const CW = 296;
const CH = 96;
            ...
            >
              ...
              <div style={{ position: 'absolute', left: 14, top: 40, width: 266, opacity: i === focus ? 1 : 0 }}>
                <div style={{ font: `500 8px/1 ${FONT}`, color: ORANGE_SOFT, letterSpacing: '.3px' }}>
                  {`${AUTHOR_LINE} · preprint:24${10 + i}.0${i % 9}${i % 7}`}
                </div>
                {[0, 1, 2, 3].map((k) => (
                  <div
                    key={k}
...
export default ResearchCardStackScroll;
