...
// [outvideo] card-sc-steep-tilt-glide —— 移植自 video-shotcraft/demos/camera/steep-tilt-glide
    ...
    <div style={{
      ...
      <FloatWrap h={liftOf(t, 0.3)}>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          <div style={{ fontSize: 66, color: INK, fontWeight: 500 }}>{((__INJ__.TEXT3 as string[])?.[2] ?? "Product analytics")}</div>
        </div>
      </FloatWrap>
      <FloatWrap h={liftOf(t, 0.42, 280)}>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <div style={{ width: 72, height: 72, borderRadius: 20, background: '#4147f5', marginLeft: 150, flexShrink: 0 }} />
          <div style={{ fontSize: 165, color: '#26262c', fontWeight: 550, marginLeft: 110, letterSpacing: 1, whiteSpace: 'nowrap' }}>{((__INJ__.TEXT2 as string[])?.[1] ?? "ClickUp 3.0")}</div>
        </div>
      </FloatWrap>
    ...
    <div style={{ position: 'absolute', left: 120, top: 560 }}>
      <FloatWrap h={liftOf(t, 0.54, 250)}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 26 }}>
          <CULogo size={104} />
          <div style={{ fontSize: 92, fontWeight: 800, color: '#222228', letterSpacing: -1 }}>{((__INJ__.TEXT as string[])?.[0] ?? "ClickUp")}</div>
        </div>
      </FloatWrap>
    </div>
