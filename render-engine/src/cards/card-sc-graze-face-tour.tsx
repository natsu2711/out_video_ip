// [outvideo] card-sc-graze-face-tour —— 移植自 video-shotcraft/demos/camera/graze-face-tour
  ...
  return (
    <div style={{ width: 2900, height: 2400, background: '#f5f5f4', display: 'flex' }}>
      <div style={{ width: 1500, borderRight: `4px solid ${FAINT}`, paddingTop: 60, background: '#f2f2f1' }}>
        ...
        <div style={{ height: 90 }} />
        <FloatWrap h={L(4)}>
          <div style={{ paddingLeft: 48, fontFamily: FONT, fontSize: 46, letterSpacing: 6, color: MID, fontWeight: 600 }}>{((__INJ__.TEXT3 as string[])?.[2] ?? "SPACES")}</div>
        </FloatWrap>
        <div style={{ height: 30 }} />
      ...
      <div style={{ flex: 1, paddingTop: 100, paddingLeft: 110 }}>
        <FloatWrap h={liftOf(t, 0.3, 150)}>
          <div style={{ display: 'flex', gap: 90, fontFamily: FONT, fontSize: 52 }}>
            <div style={{ color: INK, fontWeight: 700 }}>{((__INJ__.TEXT2 as string[])?.[1] ?? "Recent")}</div>
            <div style={{ color: MID }}>{((__INJ__.TEXT as string[])?.[0] ?? "Favorites")}</div>
          </div>
        </FloatWrap>
        <div style={{ height: 64 }} />
