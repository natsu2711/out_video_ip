// [outvideo] card-sc-neon-triple-marquee —— 移植自 video-shotcraft/demos/outro/neon-triple-marquee
...
// TEXT = 三行 marquee 词组 [第1行, 第2行, 第3行]
const ROW_WORDS = ((__INJ__.TEXT as string[])?.length ? (__INJ__.TEXT as string[]) : ['BETTER', 'FASTER', 'STRONGER']);
...
export const NeonTripleMarquee: React.FC = () => {
  ...
  const rows: { word: string; color: string; dir: 1 | -1; speed: number }[] = [
    { word: ROW_WORDS[0] ?? 'BETTER', color: '#4d9fff', dir: 1, speed: 14 },
    { word: ROW_WORDS[1] ?? 'FASTER', color: '#ff4dd2', dir: -1, speed: 17 },
    { word: ROW_WORDS[2] ?? 'STRONGER', color: '#ffb347', dir: 1, speed: 14 },
  ];
...
export default NeonTripleMarquee;
