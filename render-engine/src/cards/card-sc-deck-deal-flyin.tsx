// [outvideo] card-sc-deck-deal-flyin —— 移植自 video-shotcraft/demos/ui-entrance/deck-deal-flyin
...
import { PageCam2D, CamKey2D } from './_sc/PageCam2D';
import layout from '../../_textures/live-layout.json';

export const DECK_DEAL_FLYIN_DURATION = 113;

const __INJ__: any = (globalThis as any).__OUTVIDEO_CARD__ ?? {};
const BG_TEX = String((__INJ__.CONFIG as any)?.image_1 ?? 'textures/live/projects-empty.png');
const CARD_TEX = (__INJ__.CONFIG as any)?.image_2 ? String((__INJ__.CONFIG as any).image_2) : null;
...
export const DeckDealFlyin: React.FC = () => {
  ...
  return (
    <PageCam2D src={BG_TEX} pageH={PAGE_H} keys={CAM_KEYS}>
        ...
        return (
          <div key={`${c.file}-${i}`} style={{ transformStyle: 'preserve-3d' }}>
            {showGhost ? (
              ...
              >
                <Img src={staticFile(String(CARD_TEX ?? `textures/live/${c.file}`))} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', display: 'block' }} />
              </div>
            ) : null}
