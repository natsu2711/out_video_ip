// [outvideo] card-sc-countdown-arc-scatter —— 移植自 video-shotcraft/demos/typography/countdown-arc-scatter
...
const WORDS: { text: string; mr: number; win: [number, number] }[] = TITLE_WORDS.map((text, i) => ({
  text,
  mr: [11, 11, 0][i] ?? 11,
  win: ([[0.54, 0.68], [0.62, 0.78], [0.7, 0.9]][i] ?? [0.7, 0.9]) as [number, number],
}));

const mix = (k: number, a: number, b: number) => Math.round(lerp(k, a, b));

const __INJ__: any = (globalThis as any).__OUTVIDEO_CARD__ ?? {};
// 文字槽：TEXT = 标题逐词数组（min/to/install）；TEXT2 = 落位数字/字符（默认 "5"）
const TITLE_WORDS: string[] = ((__INJ__.TEXT as string[])?.length ? (__INJ__.TEXT as string[]) : ['min', 'to', 'install']);
const LANDING_CHAR: string = (__INJ__.TEXT2 as string[])?.[0] ?? '5';
...
export default CountdownArcScatter;
