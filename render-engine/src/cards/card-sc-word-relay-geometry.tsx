// [outvideo] card-sc-word-relay-geometry —— 移植自 video-shotcraft/demos/typography/word-relay-geometry
...
// 文字槽：TEXT = 接力词数组（3 个词依次接力，动效分 3 段不变）
const RELAY_WORDS: string[] = ((__INJ__.TEXT as string[])?.length ? (__INJ__.TEXT as string[]) : ['Faster', 'Better', 'Stronger']);
...
const SLOTS: { label: string; geom: Geom[]; t0: number; t1: number }[] = [
  { label: RELAY_WORDS[0] ?? 'Faster', geom: [{ x: 0, r: CIRC_R + 22, dash: true }], t0: 0.0, t1: 0.36 },
  {
    label: RELAY_WORDS[1] ?? 'Better',
    geom: [
      { x: -110, r: 62, d: 0 },
      { x: 0, r: 62, d: 0.06 },
      { x: 110, r: 62, d: 0.12 },
  ...
  { label: RELAY_WORDS[2] ?? 'Stronger', geom: [], t0: 0.64, t1: 1.0 },
];
...
export default WordRelayGeometry;
