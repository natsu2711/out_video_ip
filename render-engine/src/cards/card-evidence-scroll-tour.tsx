  ...
  return (
    <AbsoluteFill style={{
      ...
      <div className="doc-window">
        <div className="doc-titlebar"><i /><i /><i /><span className="fname">{(__INJ__.TEXT?.[0] ?? "个人借款服务协议（2024 修订版）.pdf")}</span></div>
        <div className="doc-viewport">
          <div className="doc-page" style={{ transform: `translateY(${y}px)` }}>
            <h1>{(__INJ__.TEXT?.[1] ?? "个人借款服务协议")}</h1>
            <div className="meta">{(__INJ__.TEXT?.[2] ?? "合同编号：2024-XJD-1107 · 甲方：某消费金融有限公司")}</div>
            <div className="rule" />

            <h2>{(__INJ__.TEXT?.[3] ?? "第一条　借款金额与期限")}</h2>
            ...
            <h2>{(__INJ__.TEXT?.[4] ?? "第二条　利息与综合费用")}</h2>
            <div className="bar w92" /><div className="bar lt w85" />
            <div className="clause"><span className="mark-wrap"><span className="mark-box" />{(__INJ__.TEXT?.[5] ?? "综合年化费率以签约页面实际展示为准")}</span>{(__INJ__.TEXT?.[6] ?? "，并可能包含服务费。")}</div>
            <div className="bar w85" /><div className="bar lt w78" /><div className="bar w66" /><div className="bar lt w40" />

            <h2>{(__INJ__.TEXT?.[7] ?? "第三条　逾期与违约责任")}</h2>
            <div className="bar w92" /><div className="bar lt w85" /><div className="bar w78" /><div className="bar lt w66" />
            <div className="clause"><span className="mark-wrap"><span className="mark-box" style={{ transform: `scale(${boxScale})`, transformOrigin: "50% 50%" }} />{(__INJ__.TEXT?.[8] ?? "逾期费率按日 1%，且不设累计上限")}</span>{(__INJ__.TEXT?.[9] ?? "，自逾期之日起计收。")}</div>
            <div className="bar w85" /><div className="bar lt w52" /><div className="bar w66" />

            <h2>{(__INJ__.TEXT?.[10] ?? "第四条　个人信息授权")}</h2>
            ...
            <h2>{(__INJ__.TEXT?.[11] ?? "第五条　争议解决")}</h2>
