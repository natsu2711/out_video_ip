// [outvideo] card-sc-dashboard-glow-highlight-pill —— 移植自 video-shotcraft/demos/effects/dashboard-glow-highlight-pill
        ...
        >
          ...
          >
            ...
            <div style={{ height: '9%', borderBottom: '1px solid #1a1c20', display: 'flex', alignItems: 'center', padding: '0 8px', gap: 10 }}>
              <span style={{ fontSize: 8, fontWeight: 800, color: '#e8e6df', letterSpacing: 1 }}>{((__INJ__.TEXT3 as string[])?.[2] ?? "◆ ACME")}</span>
              <span style={{ ...FS(6), color: '#9aa0aa' }}>{((__INJ__.TEXT2 as string[])?.[1] ?? "Trade")}</span>
              <span style={{ ...FS(6), color: '#565c66' }}>{((__INJ__.TEXT as string[])?.[0] ?? "Earn")}</span>
...
export default DashboardGlowHighlightPill;
