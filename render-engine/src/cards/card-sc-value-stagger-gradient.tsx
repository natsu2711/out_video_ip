// [outvideo] card-sc-value-stagger-gradient —— 移植自 video-shotcraft/demos/ui-entrance/value-stagger-gradient
  ...
  return (
    <DesignStage bg="#0a0b10">
      ...
      >
        ...
        >
          {t < 0.52
            ? ((__INJ__.TEXT as string[])?.[0] ?? 'scale: stagger([1, 0.35])  hue: stagger([200, 320])')
            : ((__INJ__.TEXT2 as string[])?.[0] ?? "pulse: stagger([.06, .42], { from: 'center' })")}
        </div>
      </div>
    </DesignStage>
  );
...
export default ValueStaggerGradient;
