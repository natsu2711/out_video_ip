// [outvideo] card-sc-typing-code-block —— 移植自 video-shotcraft/demos/typography/typing-code-block
...
import { DesignStage, E, seg, useT } from './_sc/Motion';
...
// 文字槽：TEXT = 代码行数组（整行着色，token 结构简化）；TEXT2/TEXT3 = 左右面板标签
const INJ_LINES = (__INJ__.TEXT as string[])?.length ? (__INJ__.TEXT as string[]) : null;
// 每行是 [text, color] token 序列
const LINES: [string, string][][] = INJ_LINES
  ? INJ_LINES.map((line) => [[line, ID]])
  : [
...
export const TypingCodeBlock: React.FC = () => {
  ...
  return (
    <DesignStage bg="#0a0b10">
      {/* 左：逐行淡入上浮（行级 stagger） */}
      <Panel x={3.5} label={(__INJ__.TEXT2 as string[])?.[0] ?? "LINE FADE-IN"}>
        {LINES.map((line, i) => {
      ...
      <Panel x={51.5} label={(__INJ__.TEXT3 as string[])?.[0] ?? "CHAR TYPING"}>
        {LINES.map((_, row) => (
