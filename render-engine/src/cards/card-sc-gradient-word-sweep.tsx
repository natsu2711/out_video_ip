// [outvideo] card-sc-gradient-word-sweep —— 移植自 video-shotcraft/demos/typography/gradient-word-sweep
      ...
      >
        <div style={lineStyle}>
          <span style={{ position: 'relative', display: 'inline-block' }}>
            <span>{((__INJ__.TEXT2 as string[])?.[1] ?? "Supercharged")}</span>
            {/* AE 式辉光：大半径柔光层（最糊） */}
            <span
              aria-hidden
              style={{
          ...
          <span>{((__INJ__.TEXT as string[])?.[0] ?? "performance")}</span>
        </div>
...
export default GradientWordSweep;
