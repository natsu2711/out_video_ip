// [outvideo] card-sc-floating-glossy-label-pills —— 移植自 video-shotcraft/demos/ui-entrance/floating-glossy-label-pills
...
import { DesignStage, E, lerp, rand, seg, useT } from './_sc/Motion';
...
// [outvideo] Slot Replacement：GROUPS[i].Body 改为图片槽优先——
// __INJ__.SLOTS[i].image 存在 → 整块渲染真实截图（md 口径：wireframe 落地时整块换截图）；
// 无图 → 原 wireframe 骨架照画（动效不变）。胶囊文案走 __INJ__.TEXT。
const SLOTS_IN: any[] = ((__INJ__.SLOTS as any[])?.length ? (__INJ__.SLOTS as any[]) : []);
const LABELS_IN: string[] = ((__INJ__.TEXT as string[])?.length ? (__INJ__.TEXT as string[]) : []);
const Shot: React.FC<{ i: number; W: number; H: number; Fallback: React.FC }> = ({ i, W, H, Fallback }) => {
  const img: string | undefined = SLOTS_IN[i]?.image;
  if (img) {
    const src = /^https?:/.test(img) ? img : staticFile(img);
    return <img src={src} alt="" style={{ width: W, height: H, objectFit: 'cover', display: 'block' }} />;
  }
  return <Fallback />;
};
...
const GROUPS: { txt: string; Body: React.FC }[] = [
  { txt: LABELS_IN[0] ?? 'Feature A', Body: BCards },
  { txt: LABELS_IN[1] ?? 'Feature B', Body: BDash },
  { txt: LABELS_IN[2] ?? 'Feature C', Body: BTable },
  { txt: LABELS_IN[3] ?? 'Feature D', Body: BForm },
];
          ...
          return (
            ...
            >
              ...
              >
                ...
                >
                  <Shot i={i} W={CW} H={CH} Fallback={Body} />
                </div>
              </div>
...
export default FloatingGlossyLabelPills;
