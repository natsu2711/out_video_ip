// [outvideo] card-sc-orbit-ring-title-open —— 移植自 video-shotcraft/demos/opening/orbit-ring-title-open
...
const TileMetric: React.FC = () => {
  ...
  return (
    <Pad>
      <div style={{ fontSize: 30, color: INK_DIM, letterSpacing: '0.06em' }}>{((__INJ__.TEXT2 as string[])?.[1] ?? "SESSIONS")}</div>
      <div
        style={{
          fontSize: 132,
          fontWeight: 700,
...
const TileImpact: React.FC = () => {
  ...
  return (
    <Pad>
      ...
      >
        {/* 常驻小行 = 首帧的锚，大字砸下来才有对比 */}
        <div style={{ fontSize: 30, color: INK_DIM, letterSpacing: '0.4em' }}>{((__INJ__.TEXT as string[])?.[0] ?? "2026 · Q3")}</div>
        <div
          style={{
            fontSize: 128,
            fontWeight: 800,
...
export default OrbitRingTitleOpen;
