  ...
  return (
    <AbsoluteFill style={{
      ...
      <div className="quote-card">
        <div className="doc-head">{(__INJ__.TEXT?.[0] ?? "《2024 年度宏观经济报告》 · 第 42 页")}</div>
        <div className="quote-line dim" style={{ opacity: dimOpacity }}>{(__INJ__.TEXT?.[1] ?? "过去三年，居民部门的储蓄率持续攀升，")}</div>
        <div className="quote-line dim" style={{ opacity: dimOpacity }}>{(__INJ__.TEXT?.[2] ?? "消费意愿始终徘徊在低位。报告指出，")}</div>
        ...
        <div className="quote-line dim" style={{ opacity: dimOpacity }}>{(__INJ__.TEXT?.[3] ?? "这一判断与多家机构的调研结论一致，")}</div>
        <div className="quote-line dim" style={{ opacity: dimOpacity }}>{(__INJ__.TEXT?.[4] ?? "政策端的回应也在陆续落地。")}</div>
      </div>
    </AbsoluteFill>
  );
}
