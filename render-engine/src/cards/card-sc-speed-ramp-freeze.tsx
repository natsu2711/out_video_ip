// [outvideo] card-sc-speed-ramp-freeze —— 移植自 video-shotcraft/demos/rhythm/speed-ramp-freeze
...
// 图片槽：CONFIG.image_1 = 被圈注的目标卡图片
const TARGET_IMG = String(((__INJ__.CONFIG as any)?.image_1) ?? 'textures/live/card4-hires.png');
      ...
      <div style={{ position: 'absolute', top: 370, transform: `translateX(${-dx + X0}px)` }}>
        {Array.from({ length: 14 }).map((_, k) => {
          ...
          return (
            <Img
              key={k}
              src={staticFile(isTarget ? TARGET_IMG : `textures/live/${c.file}`)}
              style={{
...
export default FreezeAnnotateReal;
