// [outvideo] card-sc-smear-multiples —— 移植自 video-shotcraft/demos/rhythm/smear-multiples
      ...
      {ghostOps.map((baseOp, i) => {
        ...
        return (
          <div key={k} style={{ position: 'absolute', left: gx, top: Y, opacity: op }}>
            <Card slot={0} w={480} h={320} seed={5} />
          </div>
        );
      })}
      <div style={{ position: 'absolute', left: bodyX, top: Y }}>
        <Card slot={1} w={480} h={320} seed={5} />
      </div>
    </div>
  );
};
