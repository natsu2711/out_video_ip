...
// [outvideo] card-sc-integration-hub-map —— 移植自 video-shotcraft/demos/ui-entrance/integration-hub-map
...
const HubPanel: React.FC<{ glow: number }> = ({ glow }) => (
  ...
  >
    <div style={{ flex: 2 }}>
      <div style={{ fontSize: 27, fontWeight: 600, color: '#2f2f38' }}>{((__INJ__.TEXT3 as string[])?.[2] ?? "Enterprise MQLs")}</div>
      ...
      <div style={{ fontSize: 12, color: '#8b8b92', marginTop: 14 }}>{((__INJ__.TEXT2 as string[])?.[1] ?? "Recent")}</div>
      {LIST.map((it, i) => (
    ...
    <div style={{ flex: 1, borderLeft: '1px solid #e7e7e5', paddingLeft: 22 }}>
      ...
      <div style={{ fontSize: 10.5, color: '#adadb2', marginTop: 24, letterSpacing: 1 }}>{((__INJ__.TEXT as string[])?.[0] ?? "QUICK FILTERS")}</div>
